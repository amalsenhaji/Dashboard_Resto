<template>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div
                    ...
                    <div
                        ...
                        <button type="button" @click="loadCategory()"
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Category Description</th>
                                <th scope="col" class="px-4 py-3">Public Category?</th>
                        ...
                        <tbody>
                            <tr class="border-b dark:border-gray-700" v-for="(category, index) in filtredCategories"
                                :key="index">
                                <td class="px-4 py-3">{{ category.id }}</td>
                                <td class="px-4 py-3">{{ category.name }}</td>
                                <td class="px-4 py-3">{{ category.description }}</td>
                                <td class="px-4 py-3">{{ category.business_id ? 'No' : 'Yes' }}</td>
    ...
    <!-- Main modal -->
    <CategoriesModal v-if="Category" v-model="Category" @close="close" @create="createCategory" @update="updateCategory" />
</template>
<script setup>
...
const filtredCategories = computed(() => {
    if (searchFilter.value) {
        return Categories.value.filter(item => item.name.toLowerCase().includes(searchFilter.value.toLowerCase()));
    } else {
...
const updateCategory = async () => {
    await useSupabaseClient().from('categories').update(Category.value).eq('id', Category.value.id)
...
const createCategory = async () => {
    const { data, error } = await useSupabaseClient().from('categories').insert({...Category.value, business_id: useBusiness().getBusiness.id}).select("*").single()
...
const deleteCategory = async (category, index) => {
    if (confirm('Are you sure you want to delete this category?')) {
        await useSupabaseClient().from('categories').delete().eq('id', category.id)
